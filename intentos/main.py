...
# ==========================================
# 1. ARQUITECTURA DEL MODELO (Refactorizada)
# ==========================================
...
class ARSpinViT(nk.models.AbstractARNN):
    """
    Implementación robusta de AR-ViT.
    """
    embedding_d: int
    n_heads: int
    n_blocks: int
    ...
    def conditionals(self, inputs):
        """
        Calcula la probabilidad condicional para cada espín.
        """
        ...
        for i in range(self.n_blocks):
            ...
            y = nn.Dense(self.embedding_d * 4, param_dtype=REAL_DTYPE)(y)
            y = nn.gelu(y)
            y = nn.Dense(self.embedding_d, param_dtype=REAL_DTYPE)(y)
            x = x + y

        x = nn.Dense(self.machine_pow, param_dtype=REAL_DTYPE, name="out")(x)
        return nk.nn.log_cosh(x)

    def __call__(self, inputs):
        """
        Sobrescritura manual para evitar errores de NetKet.
        """
...
# ==========================================
# 2. CONFIGURACIÓN DEL SISTEMA FÍSICO
# ==========================================

def run_experiment():
    print(">>> Iniciando experimento limpio...")
    ...
    print(">>> Calculando energía exacta (para comparar)...")
    ...
    gs.run(n_iter=100, out=log, show_progress=True)
